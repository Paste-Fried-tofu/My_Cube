...
namespace rubik_cube{
    ...
    void solver::set_thread(int num){
        thread_num = num;
    }
    ...
    solver::solver(){
        ...
        init_heuristic<true>(phrase2_corners, &solver::encode_phrase2_corners);

        std::memset(phrase2_edges1, 0xff, sizeof(phrase2_edges1));
        init_heuristic<true>(phrase2_edges1, &solver::encode_phrase2_edges1);

        std::memset(phrase2_edges2, 0xff, sizeof(phrase2_edges2));
        init_heuristic<true, true>(phrase2_edges2, &solver::encode_phrase2_edges2, &states);
        ...
        init_heuristic<false>(phrase1_edges, &solver::encode_phrase1_edges, nullptr, states);
        ...
        init_heuristic<false>(phrase1_co, &solver::encode_phrase1_co, nullptr, states);
        ...
        init_heuristic<false>(phrase1_eo, &solver::encode_phrase1_eo, nullptr, states);
    }

    template<int N, int S>
    inline int solver::encode_perm(const int8_t *p, const int *k) 
        {
            int pos[N], elem[N];
            for(int i = 0; i != N; ++i)
                pos[i] = elem[i] = i;
    ...
    int solver::encode_phrase1_edges(const cube& c){
    ...
    int solver::encode_phrase1_co(const cube& c){
    ...
    int solver::encode_phrase1_eo(const cube& c){
        block_info_ct eb = c.getEdgeBlock();
        int code = 0;
        for(int i=0; i<8; i++){
    ...
    int solver::encode_phrase2_corners(const cube& c){
        block_info_ct cb = c.getCornerBlock();
        return encode_perm<8, 7>(cb.first, factorial_8);
    }
    int solver::encode_phrase2_edges1(const cube& c){
        block_info_ct eb = c.getEdgeBlock();
        int8_t perm[8];
        for(int i = 4; i != 12; ++i)
    ...
    int solver::encode_phrase2_edges2(const cube& c){
        block_info_ct eb = c.getEdgeBlock();
        return encode_perm<4, 3>(eb.first, factorial_4);
    }
    template <int Phrase>
    int solver::estimate(const cube& c)const{
        if(Phrase == 1){
    ...
    int solver::estimate_phrase1(const cube& c)const{
        return std::max(
    ...
    int solver::estimate_phrase2(const cube& c)const{
        return std::max(
    ...
    template<int Phrase>
    bool solver::search_phrase(const search_info& s) const{
    ...
    void solver::init_permutation(int now, int8_t* v, const PushFunc& push, int avail)
    {
	...
	for(int depth = 0; ; ++depth)
	{
		...
		if(depth < 7 || thread_num == 1) 
		{
			...
			if(search_multi_thread(thread_num, s, std::bind(&solver::search_phrase<1>, this, _1)))
			{
				solution = *s.move;
				break;
			}
